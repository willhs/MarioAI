...
//		System.out.println(getParams().getRandomGenerator().nextDouble());
	...
	private static void mutate(NeatParameters params, List<Specie> species,
		...
		for (MutationOperation mo : params.getMutationOperators()) {
			int mutsPerformed = mo.mutate(params, innovations,
					fitnessScores, newOrganisms, generationNumber);
			if (mo instanceof AddConnectionMutationOperation) {
				totalAddConns += mutsPerformed;
			} else if (mo instanceof AddNeuronMutationOperation) {
