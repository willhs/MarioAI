...
public class SimpleSpeciator implements Speciator {
	...
	public double compare(Organism one, Organism two) {
		...
		while ((g1 < one.getConnections().size() - 1 && !one.getConnections().isEmpty())
				|| (g2 < two.getConnections().size() - 1 && !two.getConnections().isEmpty())) {
			// we've reached the end of genome1 but not genome2 so increment
			// the excess score
			if (g1 == one.getConnections().size() - 1 || one.getConnections().isEmpty()) {
				++g2;
				++NumExcess;

				continue;
			} else if (g2 == two.getConnections().size() - 1 || two.getConnections().isEmpty()) {
				++g1;
				++NumExcess;
		...
		double matchedScore = NumMatched != 0 ? weightDifference / NumMatched : 0;
