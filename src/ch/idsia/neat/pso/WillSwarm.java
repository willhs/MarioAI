...
public class WillSwarm {
    ...
    private List<WillParticle> _swarm = new ArrayList<>();
    private Random _random = new Random();

    public WillSwarm(MarioProblem problem) {
        for (int i = 0; i < numberOfParticles(); ++i) {
            WillParticle p = getParticle(i);
            for (int j = 0; j < p.getSize(); ++j) {
                double position =
                p.setPosition(j, position);

                double velocity = pso.Math.Scale(0, 1, getRandom().nextDouble(),
    ...
    public void addParticle(WillParticle p) {
        _swarm.add(p);
    }
    ...
    public void iterate() {
        ...
        for (int i = 0; i < numberOfParticles(); ++i) {
            WillParticle p_i = getParticle(i);
